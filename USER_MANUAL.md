...
- **Add unit** — pick **Kind** (Planes, Vehicles, Trains, Ships, Fixed Units, User Added), then a **Type** (fighter, bomber, light flak, …), then a model from the button grid. Those pickers sit above the formation view. The builder starts empty. Picture, type, cruise speed, start status (altitude, engine running, warm start, or cold start), **payload**, and **modifications** show in the formation view. Click a seat on the diagram to see that unit instead. **Add unit** appends a row: **UNIT**, then the order tree, then **+ Command / + Event / + Report / + Special**. Click the unit name button to change the model to any other unit of the same kind (all Planes, all Vehicles, …), the same way **+ Command** opens a picker. Each unit has a **Role**: Independent (own orders, can spawn), Lead, or Follows a lead. If a Lead is already set, each newly added unit follows it and `NumberInFormation` (`#`) is numbered 0, 1, 2, … **Up / Down** on the selected unit (or in Unit details) move that seat up or down in the list; Follows / Cover / Attack / Also-apply indexes stay pointed at the same units. **Copy attributes to all** copies country, skill, fuel, and flags from the selected unit onto every other unit; **payload and modifications copy only to the same aircraft type** (role, orders, and events stay). Adding another of the same type inherits that type’s payload and mods. Click a seat to highlight it. × on an order or event chip removes it. Order delay is baked into the chain (one order at a time, with a timer between). Reports wait on spawn / attack / takeoff / land instead of the previous timer.
...
- **Waypoints** — **speed** (km/h) and **altitude**. Count comes from **Goto WP** orders (one order per hop; **New** adds the next). Speed defaults to 90% of the slowest unit’s cruise, rounded to 10 km/h. Path hops are spaced **4000 m** north of the origin (move them by hand in the editor). WP 1…n are map objects with **Area 300 m** for aircraft and **100 m** for ground-only groups. There is no leftover **WP DELAY**; unused AttackArea timers are not written as orphans. **Altitude** follows the first plane’s spawn height until you enter a value; a Goto WP order can override that hop the same way. Time on Target stacks under the waypoint or attack it belongs to; events and reports stack on top.
