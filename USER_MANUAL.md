...
- **Load group…** — open a `.Group` to edit it in this mode. Files this builder already wrote (`Logic` with `ENABLE / PULSE IN`, `Zone IN`, `MISSION END`, and a `Units` subgroup) load seats, orders, zones, and bring-up as they are. Any other layout is **rebuilt** from the units and command MCUs (AttackArea, Formation, waypoints, …); proximity logic and formation spacing are laid out again on **Generate File**. The status line lists everything that was dropped or corrected (icons, extra checkzones, NodeGates, unmapped OnEvents, positions that were not a Template Builder formation, renamed Zone In, …). **Reset** clears the builder so you can author a new template from scratch (the catalog is kept).
...
- **Add unit** — pick **Kind** (Planes, Vehicles, Infantry, Trains, Ships, Fixed Units, User Added), then a **Type** (fighter, bomber, light flak, …) or, for **Infantry**, a **Country** (DPRK, PRC, USA — from the prototype’s `Country`), then a model from the button grid. Infantry pictures all use `assets/models/infantry.png`. Those pickers sit above the formation view. The builder starts empty. Picture, type, cruise speed, start status (altitude, engine running, warm start, or cold start), **payload**, and **modifications** show in the formation view. Click a seat on the diagram to see that unit instead. **Add unit** appends a row: **UNIT**, then the order tree, then **+ Command / + Event / + Report / + Special**. Click the unit name button to change the model to any other unit of the same kind (all Planes, all Vehicles, all Infantry, …), the same way **+ Command** opens a picker. Each unit has a **Role**: Independent (own orders, can spawn), Lead, or Follows a lead. If a Lead is already set, each newly added unit follows it and `NumberInFormation` (`#`) is numbered 0, 1, 2, … **Up / Down** on the selected unit (or in Unit details) move that seat up or down in the list; Follows / Cover / Attack / Also-apply indexes stay pointed at the same units. **Copy attributes to all** copies country, skill, fuel, and flags from the selected unit onto every other unit, plus altitude between planes (never above each plane’s own ceiling); **payload and modifications copy only to the same aircraft type** (role, orders, and events stay). Adding another of the same type inherits that type’s payload and mods. Click a seat to highlight it. × on an order or event chip removes it. Order delay is baked into the chain (one order at a time, with a timer between). Reports wait on spawn / attack / takeoff / land instead of the previous timer.
- **Per seat** — country (alliance), skill (0–4), fuel, **payload** (one loadout from `assets/Payloads.txt`, with an ordinance description), **modifications** (checkboxes or exclusive slots; written as IL-2 `ModMask` binary `1` / `11` / `10101` …). An aircraft can have both (F-51D bombs/rockets plus gunsight / radar / mirror / fuel / aerobatic). Vulnerable / Engageable / Limit ammo. Aircraft without a table yet keep a numeric payload id; adding rows to the text file and rebuilding picks them up. Transports such as C-47B / Li-2T use modification slots for cargo / ambulance. **AI RTB** and **Altitude** are planes only; the slider runs from 0 m to that aircraft’s reported ceiling. Above 0 m is an airstart (`StartType` 0). At 0 m the engine options appear and default to **Running** (runway, engines on); **Warm** (parked, warm engine) and **Cold** (parked, engines off) are also available. **Auto altitude (50% ceiling)** (next to **Add unit**, on by default) starts every new plane, and any plane whose model you change, in the air at half its service ceiling (MiG-15bis 7500 m, F-86A-5 7620 m). Wingmen (Follows a lead) fly at their lead’s altitude instead, capped at their own ceiling, and move with the lead when you change its altitude or, on the ground, its engine start. Switching it back on re-heights every plane. The button beside a plane’s slider does the same for that one plane: **50% ceiling** on a lead or independent plane (its wingmen follow), **Match lead** on a wingman. Turn it off to build runway or parked flights: new planes then copy the last plane’s height and start. Loading a group never changes its saved altitudes. Ground units and ships sit at 0 m and do not write plane-only keys (`AiRTBDecision`, `StartType`). **Trains** have a **Carriages** list: add, remove, and reorder cars from the catalog (the locomotive itself is the selected Train model; a matching tender is the default). Planes get flight numbers and tail codes; MiG-15bis on 501 keeps callsign 12 (Honcho).
- **Place** — formation (Inverted Vee is finger-four, also Vee, Combat Box, Pairs, Echelon, Line abreast, Column) and **how many per group**. Combat Box defaults to 6 per group; the others default to 4. Spacing is always 150 m. If a unit is a **Lead**, **In formation** sets `NumberInFormation` for that flight (0 = lead) as a modifier on the per-group count. The diagram is the same style as before, now with map-like **Zoom** (slider, scroll wheel, **Reset View**) and right-drag pan. **Zone IN / Zone Out** rings are drawn in metres (zoom out to see them). A gold **visual** ring marks the recommended combat radius for the current mix; when Zone IN sits on that radius the label reads **IN · visual**. Waypoint diamonds sit north of the origin; with a **Goto WP** order selected, click a diamond to pick that WP.
- **Zone IN** — visual range from the middle of the formation (line of sight). Defaults follow the seat mix and are not overwritten when you edit a slider, only when the mix kind changes: **aircraft** 16 km in / 35 km out (visual flag at 16 km), **ground** (vehicles, ships, fixed) 10 km in / 19 km out (visual flag at 10 km), **trains** (train-only or at least half trains) 19 km in / 30 km out (visual flag at 19 km). The empty builder starts on the aircraft pair. **visual range** appears beside a slider when its value is near that mix’s visual radius (about ±800 m), including Zone Out if you drag it there. **Zone Out** is always larger. **Trigger coalition** is who the checkzones watch: Eastern `[1]`, Western `[2]`, or both. Zone IN pulses a timer, then Activate/Spawn — not the MCU directly.
- **Orders** — **+ Command** adds Attack, AttackArea, Behavior, Cover, Effect, Flare, Force Complete, Formation, **Goto WP**, Land, and **Take Off**. **+ Special** adds **Time on Target** and **Mission Complete**. Ground units omit Cover, Land, and Take Off. **Attack** writes `MCU_CMD_AttackTarget`. **AttackArea** uses that unit’s known system range, **capped at 3000 m** (a 1 km MG gets 1000 m, not a 3 km bubble). If you enlarge the area past the weapon’s range, a warning is shown; **Match range** snaps it back. **Goto WP** is how waypoints are created: each hop is an order. Assign **WP 1**, then **New** to add the next. On arrival that WP pulses the next order’s timer (AttackArea, Time on Target, or the next Goto WP), not the MCU itself — so those timers are actually fed. The next waypoint is reached through that timer, not a WP n → WP n+1 MCU link. **Time on Target** is a timer pulsed from the waypoint before the attack (not the previous delay); when it expires, the next order fires — use this after a bomber attack so the flight is not left hanging. In the unit list, TOT stacks under the waypoint or attack it shares; reports that wait on a command stack on top of that hop like events. **OnSpawned** sits to the left of the first command, with a line, because it really does fire first. A line behind the chips shows the chain. When an event **Then**s an order, that line is broken — cleanup waits for the event instead of the previous timer. Select an order to move it with the arrows on either side. **Mission Complete** pulses **MISSION END** (Force Complete, then deactivate / delete). The previous hop starts it only when no event Then’s that chip; otherwise one of those events fires cleanup. Put Land before Mission Complete if the flight should land first. Formation types are named (Pairs, Wedge, Right, Left, Heavy Wedge, Heavy Echelon Right, Heavy Combat Box, User for aircraft; Road Column 1 way, Road Column 2 way, Panic Stop, Continue Moving for vehicles). **Cover** targets another unit. When this seat is a flight lead with wingmen, `CoverGroup` is set and the order is lead-to-lead. Independent units cover the chosen unit directly. A unit cannot cover itself. Command orders have **Also apply to**: extra units share that one MCU (`Objects`). Default is this unit only — e.g. one AttackArea on six guns, another AttackArea on two others.
...
- `MISSION END ORDERS` — Force Complete (Priority high). Loaded groups that still carry an RTB-on-zone-out order also get **RTB DELAY** then that group’s RTB waypoint (East/West). One AI order at a time.
...
1. Choose **Eastern** or **NATO** to swap the type-icon colors on this page (red vs teal). Map **Place Eastern / Place NATO** still picks which coalition parks along the front.
2. Select a type icon (**Ship**, **Armor**, **Supply**, **Artillery**, **Infantry**, or **Train**) as the default for newly added files. Each imported template then keeps its own type icon — click that template’s icons to change it. A file that contains `Train` objects is imported as **Train** even if another type is selected. A file whose vehicles are all infantry squads is imported as **Infantry**. Ship parks on water; Train parks on railroad; Armor / Supply / Artillery park on open ground. Infantry parks on dry land (not water), stays in the 10 km front band, prefers cover, and faces the closest objective (or the front when none is marked). Map draws that same type for placed units (Eastern or NATO artwork by coalition; infantry uses `assets/EasternInfantry.svg` / `NatoInfantry.svg`).
3. **Add templates…** or **Add folder…**
4. Set **Templates to create** (total copies, **max 64**). Optionally tick **Spawn all copies (omit randomizer)** so every copy starts. Otherwise set **Activate ratio (%)**. DServer often fails when too many random units fire at once — stay **under 30 random units in the whole mission** (this pack plus any others) unless spawn-all is on.
...
6. **Start delay (s)** — wait this long after Mission Begin before the first type chain (default **0**). Use this when several Army Generator groups sit in the same mission so they do not all fire at t=0. Hidden when spawn-all is on.
...
8. Optionally **Keep loaded positions** if you already placed the source file (Army Generator will not park copies on the 10 km grid; Map **Place Eastern / Place NATO** from those templates is disabled). You can still **Load Eastern / Load NATO** on Map to bring a saved army group in with or without repositioning.
9. **Generate File**, or leave Keep loaded positions off and **Place Eastern / Place NATO** on Map (Army Generator mix, one coalition at a time). To keep Eastern and NATO independent, generate each army as a `.Group` here, then load those groups on Map.
...
Use this to collect many airfields without the mission editor. Every time you start a Freeflight, the game rewrites `data\Missions\_gen.mission` with the whole generated mission. The harvester reads that file directly.
...
- **Harvest current _gen.mission** reads the file that is there now (the watcher ignores a file that existed before it was switched on).
- **Harvest a mission file…** reads any `.Mission`. With no player plane in it, every airfield in the file is harvested.
