...
4. Set **Templates to create** (total copies, **max 64**). Optionally tick **Spawn all copies (no randomizer)** so every copy starts. Otherwise set **Activate ratio (%)**. DServer often fails when too many random units fire at once — stay **under 30 random units in the whole mission** (this pack plus any others) unless spawn-all is on.
...
1. **Add packs…**
2. Check **Remove random logic (keep every copy)**.
...
- **Tools** — the strip left of the map holds six tools, also on keys **1–6**: Select AO / move units, Draw front, Salient, Attack arrow, DPRK objective, NATO objective. One tool is active at a time; the banner over the map and the status bar name it and say how to use it. **Esc** drops a half-drawn salient or arrow (or a unit WP pick) and returns to Select. An objective tool returns to Select after one placement; hold **Shift** to place more. Leaving the Map tab puts the tool down. **Ctrl Z** undoes the last drawing, or the last **Clear** / **Remove** when nothing was drawn after it; **Ctrl Y** redoes a drawing. The right dock has three tabs: **Period** (date, battle focus, suggested aircraft, clear drawn marks), **Forces** (fighters, objectives, units) and **References**.
- **Terrain** (fourth dock tab) — measured ground heights. **Height store** shows how much of the map's land is measured (the file is `%APPDATA%\IL2MissionUtility\terrain\korea_100m.hgt`). **Export survey pass…** writes 390,625 T-34 probes 800 m apart over the whole map; **Export AO tiles…** writes one probe file per 224 × 224-node tile the AO touches, 100 m apart. Import a probe file in the mission editor, select all, **set to ground**, save, then **Import snapped…** here. A file saved without set to ground is refused. **Learn from mission…** also keeps the snapped height of pinned ground units in any mission you ran set to ground on. **Coverage** shades each tile by how much of it is measured; **Relief** colours measured ground by height with hillshade. With either layer on, the map shows the ground height under the pointer. Generate does not use these heights yet. The same actions exist on the command line: `--probe-survey`, `--probe-ingest [--learn]`, `--probe-status`.
...
- **Attack arrow** — drag **from tail to tip**. Colour follows the side the tail sits on (north of the front = Eastern RGB **155, 0, 0**; south = NATO RGB **0, 120, 150**). These arrows are previewed and **exported** as `LineType` 11 (`CorrectedAttack_arrow.Group`). The **second-to-last** icon is titled **Attack**.
- **Undo** (↶ in the tool strip, **Ctrl Z**) — drops the last salient or attack arrow you drew (or cancels a stroke still in progress).
