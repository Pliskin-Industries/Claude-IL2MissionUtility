...
- UI tests: `src/ui_tests.rs` drives the app headless. Guard tests cover glyphs missing from the loaded fonts (✓ and ✗ are; use ✔ ✖), controls overflowing a side panel, wrapped button labels and 28 px targets. Add to them rather than checking by eye.
